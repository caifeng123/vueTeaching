...
/**
 * @add watch监听obj内的所有数据,变化则会调用fn
 * @tips 对于effect来说, 内部的取值引用都会被记录。但要是没有引用则不会，因此需要在effect中递归调用所有数据
 */
const watch = (source: any, fn: () => any) => {
    // @add 递归调用所有引用值, seen存放所有引用过的值
    // @add 普通值和循环引用无需被调用（ps: 否则会无限循环溢出）
    const traverse = (source: any, seen = new Set()) => {
        if (typeof source !== "object" || !obj || seen.has(source)) {
            return;
        }
        seen.add(source);
    ...
    // @add 使用traverse递归调用obj, 当被引用到的值(所有值)变了调用scheduler
    effect(() => traverse(source), {scheduler: fn});
};
